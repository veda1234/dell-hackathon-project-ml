...
def save_comment1():
    ...
    if request.method == 'POST':
        ...
        with open('newtech.csv', 'w', newline="") as inFile:
            ...
            fig1.savefig("C:/Users/Vishwajit/myproject/app/static/graph11.png")
            plt.close(fig1)
            ...
            fig2.savefig("C:/Users/Vishwajit/myproject/app/static/graph21.png")
            plt.close(fig2)
            # plt.savefig('plot')
            # plt.clf()
            # print(dds2)
            ...
            matplotlib.use('Agg')
            fig3 = plt.gcf()
            fig3.savefig("C:/Users/Vishwajit/myproject/app/static/graph31.png")
            plt.close(fig3)
            ...
            data = pd.read_csv('Newdata.csv')
            hg = np.array(data)
            if ((hg[:, 0] < 8 and hg[:, 0] > 0) and (hg[:, 1] < 2 and hg[:, 1] > -1) and (
                    hg[:, 2] < 1500 and hg[:, 2] > 0) and (hg[:, 3] > -1 and hg[:, 3] < 2) and (
                    hg[:, 4] < 3 and hg[:, 4] > 0.5) and (hg[:, 5] > 0 and hg[:, 5] < 16) and (
                    hg[:, 6] > 10 and hg[:, 6] < 20) and (hg[:, 7] > 200 and hg[:, 7] < 1000)):
                print(1)
            else:
                print(0)

            return render_template('result.html',value=Name,Y=Y,Z=Z)
